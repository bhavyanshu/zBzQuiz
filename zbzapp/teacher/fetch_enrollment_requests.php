...
if(checkIfLoggedIn($_SESSION['LoggedIn'],$_SESSION['uname'])==1) {
	//$getContentID = $_POST['getLastContentId'];
	//$i=0;
    $generate_testid = $dbinfo->prepare("SELECT teacher_course_status.uid, course_test_status.testid FROM teacher_course_status LEFT JOIN course_test_status ON teacher_course_status.courseid = course_test_status.courseid WHERE teacher_course_status.uid=? ORDER BY testid");
